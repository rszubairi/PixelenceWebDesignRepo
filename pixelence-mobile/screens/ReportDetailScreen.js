...
export default function ReportDetailScreen({ route, navigation }) {
  const { reportId, currentUser } = route.params;
  ...
  useEffect(() => {
    api.reports.getReportById({ reportId })
      .then(data => {
        setReport(data);
        if (data?.doctorComments) setDoctorComments(data.doctorComments);
      })
      .catch(() => setReport(null));
  }, [reportId]);

  const handleSaveComment = async () => {
    if (!report || !currentUser?._id) return;
    setSavingComment(true);
    try {
      await api.reports.addDoctorComment({
        reportId: report._id,
        doctorId: currentUser._id,
        doctorComments,
      });
      Alert.alert('Saved', 'Your comments have been saved.');
    } catch (err) {
      Alert.alert('Error', err.message || 'Failed to save comments');
    } finally {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        {/* Radiologist Review (read-only) */}
        {report.radiologistComments ? (
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>Radiologist Review</Text>
            <View style={[styles.contentBox, { borderLeftWidth: 3, borderLeftColor: '#3b82f6' }]}>
              {report.radiologistApproved && (
                <View style={{ backgroundColor: '#dcfce7', borderRadius: 4, paddingHorizontal: 8, paddingVertical: 2, alignSelf: 'flex-start', marginBottom: 6 }}>
                  <Text style={{ fontSize: 11, color: '#166534', fontWeight: '600' }}>APPROVED</Text>
                </View>
              )}
              <Text style={styles.contentText}>{report.radiologistComments}</Text>
            </View>
          </View>
        ) : null}
        ...
        {/* Doctor Comments */}
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Doctor Comments</Text>
          {isDoctor ? (
            <View>
              <Text style={{ fontSize: 12, color: '#6b7280', marginBottom: 8 }}>
                For system records only. Accessible to you and the patient.
              </Text>
              <TextInput
                style={styles.commentInput}
                multiline
                numberOfLines={4}
                placeholder="Add your clinical comments or follow-up notes..."
                value={doctorComments}
                onChangeText={setDoctorComments}
                textAlignVertical="top"
              />
              <TouchableOpacity
                style={[styles.saveButton, (!doctorComments.trim() || savingComment) && { opacity: 0.5 }]}
                onPress={handleSaveComment}
                disabled={!doctorComments.trim() || savingComment}
              >
                <Text style={styles.saveButtonText}>{savingComment ? 'Saving...' : 'Save Comments'}</Text>
              </TouchableOpacity>
            </View>
          ) : report.doctorComments ? (
            <View style={[styles.contentBox, { borderLeftWidth: 3, borderLeftColor: '#7c3aed' }]}>
              <Text style={styles.contentText}>{report.doctorComments}</Text>
            </View>
          ) : (
            <Text style={{ color: '#9ca3af', fontSize: 13 }}>No doctor comments yet.</Text>
          )}
        </View>
